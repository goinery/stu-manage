...
本实验以学校学生选课为场景进行应用系统开发。以vscode作为集成开发环境，Java作为应用开发语言，layui作为前端框架，SpringBoot、Mybatis作为后端框架，openGauss作为后端数据库开发一个《学生选课管理系统》，openGauss数据库安装部署于华为弹性云ECS服务器上。

### 0.1.2 读者知识背景
本课程为应用系统开发实践课，为了更好地掌握本书内容，阅读本书的读者应首先具备以下基本条件：
- 具有基本的数据库知识背景，同时熟悉华为云界面，了解基本Linux、Java知识。
